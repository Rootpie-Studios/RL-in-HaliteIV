...
import time
import pprint
import random, pickle, os, collections
import src.helper as helper
import enemy_data.conf as conf
import numpy as np
...
#############
# Gather enemy data
gameID = str(len(os.listdir(conf.get('GAMES_FOLDER'))) - 1)
with open(conf.get('GAMES_FOLDER') + '/' + gameID, 'rb') as pickle_file:
    memory = pickle.load(pickle_file)
memory.update({'enemy' : collections.deque(maxlen=401)})
...
def agent(observation, configuration):
    ...
    own_ship_actions = {}
    epsilon = conf.get('AGENT2_EPSILON')
    ##############
    ...
    for ship in me.ships:
        ...
        if ship_states[ship.id] in directions:
            ...
            action = helper.ship_action_index(ship.next_action)
        else:
            action = 0
        if epsilon > np.random.rand() and board.step > 5:
    ...
    with open(conf.get('GAMES_FOLDER') + '/' + gameID, 'rb') as pickle_file:
        memory = pickle.load(pickle_file)
    memory['enemy'].append((obs, me.next_actions, all_other_ship_actions, me.id))
